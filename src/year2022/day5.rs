...
use advent_of_code_traits::{days::Day5, ParseInput, Part1, Part2, Solution};
...
impl ParseInput<'_, Day5, Part1> for AdventOfCode2022<Day5> {
    type Parsed = (HashMap<u32, Vec<char>>, Vec<Instruction>);
...
impl Solution<'_, Day5, Part1> for AdventOfCode2022<Day5> {
    type Input = (HashMap<u32, Vec<char>>, Vec<Instruction>);

    type Output = String;

    fn solve(&self, input: &(HashMap<u32, Vec<char>>, Vec<Instruction>)) -> Self::Output {
...
impl Solution<'_, Day5, Part2> for AdventOfCode2022<Day5> {
    type Input = (HashMap<u32, Vec<char>>, Vec<Instruction>);
    ...
    fn solve(&self, input: &(HashMap<u32, Vec<char>>, Vec<Instruction>)) -> Self::Output {
        ...
        for instruction in instructions {
            ...
            let mut abc: Vec<char> = crate_chars
                .clone()
                .into_iter()
                .rev()
                .take(instruction.quantity as usize)
                .rev()
                .collect();

            new_crate_chars.append(abc.borrow_mut());
            for _ in 0..instruction.quantity {
                crate_chars.pop();
            }
